...
describe('SurveyMongoRepository', () => {
  ...
  describe('loadBySurveyId()', () => {
    test('Should load survey result', async () => {
      ...
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ surveyId: survey.id, accountId: accountId1 })
      expect(surveyResult).toBeTruthy()
    ...
    test('Should load survey result 2', async () => {
      ...
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ surveyId: survey.id, accountId: accountId2 })
      expect(surveyResult).toBeTruthy()
      expect(surveyResult.surveyId).toEqual(survey.id)
      expect(surveyResult.answers[0].count).toBe(2)
    ...
    test('Should load survey result 3', async () => {
      ...
      const sut = makeSut()
      const surveyResult = await sut.loadBySurveyId({ surveyId: survey.id, accountId: accountId3 })
      expect(surveyResult).toBeTruthy()
      expect(surveyResult.surveyId).toEqual(survey.id)
    ...
    test('Should return null if there is no survey result', async () => {
      const { id: surveyId } = await mockSurvey()
      const accountId = await mockAccountId()
      const sut = makeSut()
